...
### Виконання
Згідно з [документацією Docker](https://docs.docker.com/reference/cli/docker/container/run/#ulimit), під час запуску контейнера можна задавати обмеження ulimit, які лімітують ресурси процесів усередині нього. Це можна перевірити на простому прикладі, запустивши контейнер і спробувавши створити файл більшого розміру, ніж дозволено:

```bash
docker run --rm -it --ulimit fsize=10240 fedora:43 /bin/bash -c "dd if=/dev/zero of=test.bin bs=1K count=100"
```

У цьому випадку команда dd завершиться з помилкою на кшталт File too large, оскільки ми обмежили розмір файлу приблизно до 10 КБ. Якщо ж прибрати обмеження або встановити більше значення fsize, той самий файл у 100 КБ створиться без проблем — отже, зміна ulimit безпосередньо впливає на поведінку програм у контейнері.

## Завершення роботи

Після завершення експериментів обчислювальний екземпляр у Google Cloud Platform необхідно видалити з метою уникнення подальшого нарахування вартості за використані ресурси:

```bash
gcloud compute instances delete lab3 --zone=europe-west1-b
gcloud compute disks delete lab3 --zone=europe-west1-b
```

Практичну роботу можна вважати завершеною.
